...
# Deliberately NOT stripped to a bare prefix like the other pipelines do:
# PROCESSED_PREFIX and FAILED_PREFIX are named as suffixes on this same
# watch folder name (...-raw-samples-processed/, ...-raw-samples-failed-
# processing/), so a bare-string startswith() check would also match those
# sibling folders - confirmed live: move_blob's copy into the processed
# folder fires its own GCS "finalized" event, which without the trailing
# slash got treated as back in scope, triggering a reprocess that moved the
# file onto itself and deleted it. Keeping the trailing slash anchors the
# check to an actual folder boundary.
...
LOGS_URL = f"https://console.cloud.google.com/functions/details/europe-west2/films-tensile-raw-processor?project={PROJECT_ID}&tab=logs"
...
def send_failure_email(blob_name, error_message):
    ...
    try:
        ...
        body += email_style.paragraph(
            ...
            "the line beginning TENSILE_RAW_PIPELINE_FAILURE."
        )
        ...
        send_result = gmail_sender.send_html_email(
            ...
            f"[Alert] Tensile raw curve: {filename} failed processing", html,
        )
        print(f"TENSILE_RAW_FAILURE_ALERT_SENT file={filename} message_id={send_result.get('id')}")
    except Exception as send_exc:
        print(f"TENSILE_RAW_FAILURE_ALERT_SEND_FAILED file={blob_name} error={send_exc}")
...
def process_file(cloud_event):
    ...
    try:
        ...
        # move the file so its location reflects its state, then re-raise so
        # Eventarc and Cloud Logging record an error. Safe to re-raise since
        # the trigger uses RETRY_POLICY_DO_NOT_RETRY.
        print(f"TENSILE_RAW_PIPELINE_FAILURE file={blob_name} error={exc}")
        ...
        try:
            ...
            print(f"TENSILE_RAW_PIPELINE_FAILURE could not move to failed prefix: {move_exc}")
            print(traceback.format_exc())
