"""Replay shared/tensile_parser.py against every real processed tensile file
and confirm its row counts match what the deployed pipeline actually
...
Usage: python3 shared/verify_tensile_parser.py
"""
...
def load_manifest_by_checksum():
    ...
    query = """
        ...
        WHERE pipeline = 'tensile' AND status = 'success'
...
def main():
    print("Loading manifest (pipeline=tensile, status=success)...")
    ...
    print(f"Found {len(blobs)} processed tensile CSVs to replay.\n")
    ...
    for i, blob in enumerate(blobs, 1):
        ...
        try:
            df, rows_dropped, _ = extract_relevant_dataframe(csv_bytes, source_file=filename)
